...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const response = await fetch(
    `/v1/getUserData?userId=${userId}&driverLicence=true&vehicles=true&tickets=true&driveTest=true&license_request=true`
  );
  if (!response.ok) {
    return alert(
      "Error al buscar los datos del usuario,recargue la pagina o contacte con soporte tecnico."
    );
  }
  ...
  document.getElementById(
    "avatar"
  ).src = `https://api.cacolombia.com/images/${userdata.user_id}/user`;
  ...
  document
    .getElementById("pay-survey-form-form")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
      try {
        const response = await fetch(
          `/v1/getUserData?userId=${userId}&driverLicence=true`
        );
        if (!response.ok) {
          return alert(
            "Error al buscar los datos del usuario,recargue la pagina o contacte con soporte tecnico."
        ...
        if (!inv) {
          return alert(
            "No tienes inventario, usa el comando /saldo en discord para crearlo y reinicia la pagina."
          );
        }
  ...
  document
    .getElementById("survey-questions")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
      let points = 0;
  ...
  document
    .getElementById("license-request-form")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
      try {
        const theoretical_test_id = document.getElementById(
          "theoretical-drive-test"
        ).value;
        const practical_test_id = document.getElementById(
        ...
        if (rq.ok) {
          ...
          if (
            rq.status === 400 &&
            response.message == "Other request is waiting for response"
          ) {
  ...
  document
    .getElementById("pay-impoundment-form")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
      try {
        ...
        if (!inv) {
          return alert(
            "No tienes inventario, usa el comando /balance en discord para crearlo y reinicia la pagina."
          );
        }
        ...
        if (newMoney < 0) {
          ...
          return alert(
            "No tienes suficiente dinero para pagar la incautación."
          );
        }
        inv.money.bank = newMoney;
        ...
        if (rq.ok) {
          ...
          if (rq.status === 404) {
            alert(
              "error:Parece que no tienes un vehiculo a tu nombre con esa placa."
            );
          } else {
  ...
  document
    .getElementById("register-vehicle-form")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
      try {
        const placaRegex = /^[A-Z]{3}-\d{3}$/;
        ...
        if (r2q.ok) {
          ...
          const upditem =
            existitem.quantity > 0
              ? items
              : items.filter((item) => item.name !== model);
          inv.items = upditem;
          await editinventory(userId, inv);
          window.location.reload();
  ...
  document
    .getElementById("form-transfer")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
      const documentId = document.getElementById(
        "document-number-transfer"
      ).value;
      ...
      if (response.status === 404) {
        document.getElementById("form-transfer").reset();
        return alert(
          "Esta placa no está  a tu nombre o no existe una persona con este numero de documento"
        );
      }
      if (!response.ok) {
        document.getElementById("form-transfer").reset();
        ...
        return alert(
          "Ocurrió un problema realizando la transferencia: " + data.message
        );
      }
    });
});
...
`;
    ...
    btn.className =
      "px-3 py-1.5 bg-blue-500 text-white rounded-md shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-1 text-sm transition";
    btn.textContent = "Ver certificado";
    btn.addEventListener("click", () =>
      window.open(
        `https://app.cacolombia.com/pdfs/cursos/${curso.test_id}.pdf`,
        "_blank"
      )
    );
...
async function loadDriveTest() {
  const theoretical_select_menu = document.getElementById(
    "theoretical-drive-test"
  );
